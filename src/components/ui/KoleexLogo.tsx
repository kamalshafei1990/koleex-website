/* ---------------------------------------------------------------------------
   KoleexLogo — Inline SVG logo component with color variants.
   The SVG is taken from the official Koleex brand asset.
   --------------------------------------------------------------------------- */

interface KoleexLogoProps {
  color?: "white" | "dark" | "current";
  height?: number;
  className?: string;
}
...
export function KoleexLogo({
  color = "dark",
  height = 18,
  className,
}: KoleexLogoProps) {
  const fill = fills[color];
  // Original viewBox aspect ratio: 719.83 / 107.57 ≈ 6.69
  const width = Math.round(height * 6.69);
